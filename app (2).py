...
def call_groq_vision(messages, max_tokens=2048):
    """Call Groq Vision API for image analysis (Snap & Solve)."""
    headers = {
        "Authorization": f"Bearer {GROQ_API_KEY}",
        "Content-Type": "application/json"
    }
    payload = {
        ...
        # Llama 4 Scout is the current supported vision model.
...
def generate_image():
    ...
    try:
        ...
        # model=flux    -> higher quality generator
        # enhance=true  -> Pollinations improves vague prompts before generating
        # seed=random   -> fresh generation each time instead of a cached result
        encoded_prompt = requests.utils.quote(prompt)
        seed = random.randint(1, 999999)
        image_url = (
            f"https://image.pollinations.ai/prompt/{encoded_prompt}"
            f"?width=768&height=768&model=flux&enhance=true&nologo=true&seed={seed}"
        )
